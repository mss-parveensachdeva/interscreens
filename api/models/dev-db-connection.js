...
try {
	vcapLocal = require(path.join(__dirname,'../../config/dev-db-config.json'));
...
if (appEnv.vcap.services.cloudantNoSQLDB) {
	...
	console.log("Please check your configuration file" , path.join(__dirname,'../../config/dev-db-config.json'), JSON.stringify(appEnv));
